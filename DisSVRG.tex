...
=======
Machine learning applications such as image recognition \cite{Coates2011An}, speech recognition \cite{dahl2012context} and text processing \cite{collobert2008unified} proliferate in the era of Big Data. Those underlying machine learning models are usually complex and large. This is made possible by training many parameters from a large amount of training data. For example, it is possible to train a large scale deep neural network which consists of millions or even billions of parameters by feeding it with terabytes of training data. Furthermore, it is worth noting that most machine learning algorithms are iteratively convergent, which means those algorithms need many rounds of iterative calculations to update the parameters of their models. Considering the massive amount of computation brought about by the complexity of the underlying model and the huge size of the training data and , an efficient training method is vital for performing a large scale learning task.

Many machine learning algorithms can be described as an optimization problem like (\ref{equa_loss_minimization}).
>>>>>>> origin/master
\begin{equation}
...
=======
\end{equation}. Here, $\nabla$ stands for the derivation operation. $F(\omega)$ is generally called the loss function. $\omega$ represents the parameters of a model, that is, the parameters that needs to be updated during the iterations. $n$ means the size of training data and $t$ represents the $t$th iteration. The loss function $F(\omega)$ can be minimized by updating the parameters iteratively, which is called the learning process. Conventionally, a global average gradient is calculated, i.e., $\nabla F(\omega_{t-1})$, and then used to update the parameters during an iteration. Since$\nabla F(\omega_{t-1})$ needs $n$ derivations, which are time-consuming, the gradient descent method is not practical for a large-scale learning task.  An alternative approach  is Stochastic  Gradient Descent (SGD) and its variants. SGD randomly samples an instance from the training data, and then uses it to compute the local gradient, i.e., $\nabla f_i(\omega_{t-1})$, rather than the global average gradient.   The parameters are thus updated with the local gradient. Since $\nabla f_i(\omega_{t-1})$ merely needs one derivation for the local gradient in each iteration,  it is efficient for large-scale learning tasks.  However, since $f_i$ is randomly sampled, variances exists between the local gradient $\nabla f_i(\omega_{t-1})$ and the global average gradient $\nabla F(\omega_{t-1})$, which slows the convergence of the loss function i.e., $F(\omega)$, of a machine learning algorithm.  Specifically, if the parameters are close to the optimal state, it is difficult to decrease the loss function due to the variance. Conventionally, the variance is reduced by using a decaying learning rate to update the parameters. Although the variance can be reduced by the decaying learning rate, a small learning rate unavoidably  leads to slow convergence.

Recently, the SGD and its variants have been widely used to train the parameters of a model in distributed memory systems \cite{Dean:2012wx, Li:2014tt, Xing:2015ib}. Those versions of SGD generally train and update parameters in a parameter-server system by using a cluster. The nodes in the parameter-server system are categorized into servers and workers. The underlying calculations of the gradients are conducted by workers. Those updates will be then pushed to servers, and be aggregated on servers for updating the global parameters. Finally, those newly learned global parameters will be shared among workers. Since there exists a lot of communication between workers and servers, all distributed versions of SGD like PetuumSGD focus on the optimization of communication \cite{Xing:2015ib}.  Specifically, PetuumSGD has proposed the asynchronous communication protocol named Staleness Synchronous Protocol (SSP) to conduct communication across nodes. SSP outperforms the Bulk Synchronous Protocol (BSP) employed by Hadoop or Spark. However, since SSP is designed for  general iteratively convergent machine learning algorithms, it does not exploit the potential benefits of SGD to accelerate the iterative calculations. For example, PetuumSGD updates the parameters by using a decaying learning rate. For instance, the learning rate $\eta$ in the current iteration will become $0.95\eta$ in the next iteration. When the parameters are close to the optimal state, it is difficult to further decrease the value of the loss function due to an extremely small learning rate. In a nutshell,   even though PetuumSGD adopts SSP  to optimize the communication between workers and servers, a decaying learning rate slows the convergence.
>>>>>>> origin/master

Meanwhile, a new technique for variance reduction is proposed to speed up the convergence of SGD \cite{Johnson:9MAvkbgy, Zhao:SZfxEHHg, Reddi:2015vj}. It reduces the variance of SGD, and keeps SGD converging at a constant rate.   However, those versions of SGD are designed to be used in one single server instead of a cluster with many nodes. When the amount of parameters or the size of training data exceeds the storage capacity of  a server, the underlying variance reduction technique will not work. For instance, a deep network may have billions or even trillions of parameters, which cannot be stored in a node. Therefore, such versions of SGD are incapable of performing a large-scale training task.
...
=======
In this paper, we design a distributed and asynchronous version of variance-reduced SGD denoted by DisSVRG for large-scale machine learning tasks. DisSVRG adopts the asynchronous communication protocol, i.e., Staleness Synchronous Protocol (SSP). In order to obtain a fast convergence, DisSVRG is accelerated by using a learning rate with an acceleration factor. It is unavoidable that some fast workers will spend a lot of time on waiting for slower workers when performing iterative calculations  on a cluster, which is also known as the "straggler problem". The straggler problem results in a waste of time, thus leading to the slow convergence of a machine learning algorithm. In order to reduce the wait time, we propose an adaptive sampling strategy to alleviate the straggler problem. Specifically, we dynamically adjust the random sampling strategy during the iterations. When a worker is faster than other workers, it will  sample more instances in the next iteration, which will take more time to compute the local gradient.   Thus, slower workers will have chance to catch up with the faster works, and  the wait time can be significantly reduced. Finally, we conduct empirical studies on the \emph{Tianhe-1} supercomputer which is located in the National Supercomputing Center in Changsha. The performance evaluation verifies that DisSVRG outperforms the state-of-the-art version of SGD, and obtains approximately a linear speedup on Tianhe-1.
>>>>>>> origin/master

The rest of this paper is organized as follows. Section \ref{related_work} outlines the related work of our work. Section \ref{overview} presents the assumptions and an overview of DisSVRG. Section \ref{implementation} illustrates the details of DisSVRG. Section \ref{optimization_sgd} highlights the  optimization of DisSVRG. Section \ref{discussion} discusses the major difference between our work and  previous studies. Section \ref{performance_evaluation} shows the performance evaluation. Section \ref{conclusion} concludes the paper.
...
\label{figure_PS_and_data_parallelism}
\end{figure*}
